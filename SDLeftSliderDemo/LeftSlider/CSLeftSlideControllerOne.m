...
- (ViewControllerOne *)projectVC{
    if (_projectVC == nil) {
        _projectVC = [[ViewControllerOne alloc] init];
    }
    return _projectVC;
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(enterProgectVC) name:KNotificationTapCenterVC object:nil];
...
- (void)setupMainVC:(UIViewController *)mainVC
{
    ...
    mainVC.view.layer.shadowOffset = CGSizeMake(-3, 3);
    mainVC.view.layer.shadowOpacity = 0.6;
}

- (void)setupLeftVC:(LeftVC *)leftVC
{
    _leftVC = leftVC;
    ...
    _leftVC.view.width = kLeftViewW;
     __weak CSLeftSlideControllerOne *weakSelf = self;
    _leftVC.block = ^(NSInteger selectedIndex){
        [weakSelf leftVCRowSelectedIndex:selectedIndex];
    };
...
-(void)showLoginViewDelegat{
    self.view.userInteractionEnabled = true;
    _leftVC.view.userInteractionEnabled = true;
}
...
- (void)leftVCRowSelectedIndex:(NSInteger)index{
    ...
    switch (index) {
        ...
        case 1:
        {
            ViewControllerTwo *customVC = [[ViewControllerTwo alloc] init];
        ...
        {
            //收藏
            ViewControllerThree *favouriteVc = [[ViewControllerThree alloc] init];
            vc = favouriteVc;
        }
            break;
        case 3:
        {
            ViewControllerFour *collectVC = [[ViewControllerFour alloc] init];
            vc = collectVC;
        }
            break;
...
- (void)enterProgectVC{
    UINavigationController *nav = (UINavigationController *)_mainVC;
    [nav popToRootViewControllerAnimated:NO];
    [nav pushViewController:self.projectVC animated:NO];
}
